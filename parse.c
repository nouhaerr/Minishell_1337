...
/*   Updated: 2023/05/25 16:24:36 by hobenaba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_node(t_parser *parser, t_lexer *lexer)
{
	if (lexer -> i == 0 && (lexer->tokens2->type == word))
		...
		ft_lstaddback4(&(parser->outfiles), ft_lstnew4(lexer -> tok -> value, clear));
	else if (lexer->tokens2->type == dr_rdr)
		ft_lstaddback4(&(parser->outfiles), ft_lstnew4(lexer -> tok -> value, finish_up));
	else if (lexer->tokens2->type == heredoc && lexer->tok ->arten == general) 
		ft_lstaddback4(&(parser->heredoc), ft_lstnew4(lexer -> tok -> value, expand));
	else if (lexer->tokens2->type == heredoc && lexer->tok ->arten == quotes) 
		ft_lstaddback4(&(parser->heredoc), ft_lstnew4(lexer -> tok -> value, not_expand));
}
